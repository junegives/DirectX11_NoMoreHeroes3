...
HRESULT CGraphic_Device::Ready_Graphic_Device(const GRAPHIC_DESC& GraphicDesc, ComPtr<ID3D11Device>* ppDeviceOut, ComPtr<ID3D11DeviceContext>* ppDeviceContextOut, HWND hWnd)
{
	_uint		iFlag = 0;
...
//HRESULT CGraphic_Device::Ready_2D_Device(HWND hWnd)
//{
...
//
//	// Direct2D Factory 생성
//	HRESULT hr = D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, __uuidof(ID2D1Factory), &m_pD2DFactory);
//	if (FAILED(hr))
//		return hr;
//
//	// DirectWrite Factory 생성
//	hr = DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(IDWriteFactory), static_cast<IUnknown**>(&m_pDWriteFactory));
//	if (FAILED(hr))
//		return hr;
//
//	// 렌더 타겟을 위한 D2D 디바이스 생성
//	hr = m_pD2DFactory->CreateHwndRenderTarget(
//		D2D1::RenderTargetProperties(),
//		D2D1::HwndRenderTargetProperties(hWnd, D2D1_SIZE_U{ static_cast<UINT32>(Viewport.Width), static_cast<UINT32>(Viewport.Height) }),
//		&m_pD2DRenderTarget
//	);
//	if (FAILED(hr))
//		return hr;
...
//
//	// 텍스트 색상
//	m_pD2DRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Black), &m_pBlackBrush);
//
//	return S_OK;
//}
//
//void CGraphic_Device::Draw2DText(const WCHAR* text, _float x, _float y)
//{
//	D3D11_VIEWPORT Viewport;
//
//	_uint iNumViewPorts = 1;
...
//
//	// 텍스트를 출력할 위치 설정
//	D2D1_RECT_F layoutRect = D2D1::RectF(x, y, Viewport.Width, Viewport.Height);
//
//	// 텍스트 출력
...
shared_ptr<CGraphic_Device> CGraphic_Device::Create(const GRAPHIC_DESC& GraphicDesc, ComPtr<ID3D11Device>* ppDevice, ComPtr<ID3D11DeviceContext>* ppDeviceContextOut, HWND hWnd)
{
	shared_ptr<CGraphic_Device> pInstance = make_shared<CGraphic_Device>();

	if (FAILED(pInstance->Ready_Graphic_Device(GraphicDesc, ppDevice, ppDeviceContextOut, hWnd)))
	{
