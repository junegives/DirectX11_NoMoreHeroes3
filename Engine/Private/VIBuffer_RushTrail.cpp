...
CVIBuffer_RushTrail::CVIBuffer_RushTrail(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext)
    : CVIBuffer(pDevice, pContext)
{
}

CVIBuffer_RushTrail::CVIBuffer_RushTrail(const CVIBuffer_RushTrail& rhs)
    : CVIBuffer(rhs)
{
}

HRESULT CVIBuffer_RushTrail::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CVIBuffer_RushTrail::Initialize(void* pArg)
{
	m_iLimitPointCount = 150;
	...
	m_iNumIndices = (m_iNumVertices - 2) * 3;
...
HRESULT CVIBuffer_RushTrail::Update(TRAIL_DESC TrailPoint)
{
	_float3 UpperPos;
	_float3 LowerPos;
...
#pragma endregion 전체 구간 보간
	...
	memcpy(SubResource.pData, pVertices, sizeof(VTXPOSTEX) * m_iNumVertices);
...
HRESULT CVIBuffer_RushTrail::Clear()
{
	m_TrailPoint.clear();
...
shared_ptr<CVIBuffer_RushTrail> CVIBuffer_RushTrail::Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext)
{
	shared_ptr<CVIBuffer_RushTrail> pInstance = make_shared<CVIBuffer_RushTrail>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CVIBuffer_RushTrail");
		pInstance.reset();
	}
...
shared_ptr<CComponent> CVIBuffer_RushTrail::Clone(void* pArg)
{
	shared_ptr<CVIBuffer_RushTrail> pInstance = make_shared<CVIBuffer_RushTrail>(*this);
