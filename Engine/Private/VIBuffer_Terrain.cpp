...
HRESULT CVIBuffer_Terrain::Initialize(void* pArg)
{
	...
	int* iTerrainSize = (int*)pArg;

	m_iNumVerticesX = iTerrainSize[0];
	m_iNumVerticesZ = iTerrainSize[1];
...
_bool CVIBuffer_Terrain::Picking(_long iMouseX, _long iMouseY, _matrix matWorld, _float3* vPickPoint, _float* fMinDist)
{
	...
	for (size_t i = 0; i < m_iNumVerticesZ - 1; i++)
	{
		for (size_t j = 0; j < m_iNumVerticesX - 1; j++)
		{
			...
			if (m_pPicking->IntersectWorld(m_pVerticesPos[iIndices[0]], m_pVerticesPos[iIndices[1]], m_pVerticesPos[iIndices[2]], vPickPoint, fMinDist))
			{
				return true;
			}
			else if (m_pPicking->IntersectWorld(m_pVerticesPos[iIndices[0]], m_pVerticesPos[iIndices[2]], m_pVerticesPos[iIndices[3]], vPickPoint, fMinDist))
			{
				return true;
			}
		}
...
shared_ptr<CVIBuffer_Terrain> CVIBuffer_Terrain::Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext)
{
