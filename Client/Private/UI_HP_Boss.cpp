...
CUI_HP_Boss::CUI_HP_Boss(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext)
    : CUI(pDevice, pContext)
{
}

CUI_HP_Boss::CUI_HP_Boss(const CUI_HP_Boss& rhs)
    : CUI(rhs)
{
}

CUI_HP_Boss::~CUI_HP_Boss()
{
    Free();
}
...
HRESULT CUI_HP_Boss::Initialize(void* pArg)
{
    ...
    m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(0.f, -m_UIDesc.fY + g_iWinSizeY * 0.5f, 0.f, 1.f));

    return S_OK;
}

void CUI_HP_Boss::Priority_Tick(_float fTimeDelta)
{
}
...
void CUI_HP_Boss::Late_Tick(_float fTimeDelta)
{
    m_isVisible = true;
...
HRESULT CUI_HP_Boss::Add_Component()
{
    ...
    InstanceDesc.eUIType = CVIBuffer_Particle_Point_UI::UI_BOSSHP;
    ...
    if (FAILED(CGameObject::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_UI_Point_150"),
...
shared_ptr<CUI_HP_Boss> CUI_HP_Boss::Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext, void* pArg)
{
    shared_ptr<CUI_HP_Boss> pInstance = make_shared<CUI_HP_Boss>(pDevice, pContext);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed to Created : CUI_HP_Boss");
        pInstance.reset();
    }
