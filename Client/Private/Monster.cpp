...
HRESULT CMonster::Initialize(void* pArg)
{
    ...
    m_pPlayerTransformCom = m_pGameInstance->Get_Player()->Get_TransformCom();
...
{
    m_fDistanceToPlayer = XMVectorGetX(XMVector4Length(m_pPlayerTransformCom->Get_State(CTransform::STATE_POSITION) - m_pTransformCom->Get_State(CTransform::STATE_POSITION)));
...
_float CMonster::GetDistanceToPlayer()
{
    return m_fDistanceToPlayer;
}

shared_ptr<CMonster> CMonster::Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext, void* pArg)
{
