...
CUI_Target::CUI_Target(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext)
...
void CUI_Target::Priority_Tick(_float fTimeDelta)
{
}

void CUI_Target::Tick(_float fTimeDelta)
{
    if (!m_isVisible)
        return;
...
void CUI_Target::Late_Tick(_float fTimeDelta)
{
    m_isVisible = true;
...
HRESULT CUI_Target::Add_Component()
{
    ...
    InstanceDesc.eUIType = CVIBuffer_Particle_Point_UI::UI_TARGET;
    ...
    if (FAILED(CGameObject::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_UI_Point_20"),
...
HRESULT CUI_Target::Bind_ShaderResources()
{
...
shared_ptr<CUI_Target> CUI_Target::Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext, void* pArg)
{
    shared_ptr<CUI_Target> pInstance = make_shared<CUI_Target>(pDevice, pContext);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed to Created : CUI_Target");
        pInstance.reset();
    }
