...
class CUI_HP_Player : public CUI
{
public:
	CUI_HP_Player(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext);
	CUI_HP_Player(const CUI_HP_Player& rhs);
	virtual ~CUI_HP_Player();
...
public:
	static shared_ptr<CUI_HP_Player> Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext, void* pArg = nullptr);
