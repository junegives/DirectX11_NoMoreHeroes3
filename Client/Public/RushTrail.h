...
class CRushTrail final : public CGameObject
{
public:
	typedef struct
	{
...
public:
	CRushTrail(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext, const wstring& strTextureComTag);
	CRushTrail(const CRushTrail& rhs);
	virtual ~CRushTrail() = default;
...
private:
	...
	shared_ptr<CVIBuffer_RushTrail>	m_pVIBufferCom = nullptr;
...
public:
	static shared_ptr<CRushTrail> Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext, const wstring& strTextureComTag, void* pArg = nullptr);
